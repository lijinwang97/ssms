...
srs_error_t SrsHybridServer::on_timer(srs_utime_t interval) {
    ...
    if (_srs_pps_timer->r10s() || _srs_pps_pub->r10s() || _srs_pps_conn->r10s()) {
        snprintf(buf, sizeof(buf), ", timer=%d,%d,%d", _srs_pps_timer->r10s(), _srs_pps_pub->r10s(),
                 _srs_pps_conn->r10s());
        timer_desc = buf;
    }
    ...
    if (_srs_pps_clock_15ms->r10s() || _srs_pps_timer_s->r10s() || _srs_pps_clock_20ms->r10s() ||
        _srs_pps_clock_25ms->r10s() || _srs_pps_clock_30ms->r10s() || _srs_pps_clock_35ms->r10s() ||
        _srs_pps_clock_40ms->r10s() || _srs_pps_clock_80ms->r10s() || _srs_pps_clock_160ms->r10s()) {
        snprintf(buf, sizeof(buf), ", clock=%d,%d,%d,%d,%d,%d,%d,%d,%d", _srs_pps_clock_15ms->r10s(),
                 _srs_pps_clock_20ms->r10s(), _srs_pps_clock_25ms->r10s(), _srs_pps_clock_30ms->r10s(),
                 _srs_pps_clock_35ms->r10s(), _srs_pps_clock_40ms->r10s(), _srs_pps_clock_80ms->r10s(),
                 _srs_pps_clock_160ms->r10s(), _srs_pps_timer_s->r10s());
        clock_desc = buf;
    }
    ...
    if (_srs_pps_objs_rtps->r10s() || _srs_pps_objs_rraw->r10s() || _srs_pps_objs_rfua->r10s() ||
        _srs_pps_objs_rbuf->r10s() || _srs_pps_objs_msgs->r10s() || _srs_pps_objs_rothers->r10s()) {
    ...
    /*srs_trace("Hybrid cpu=%.2f%%,%dMB%s%s%s%s%s%s%s%s%s%s%s", //注释
    ...
    SrsJsonObject *base = SrsJsonAny::object();
    base->set("app", SrsJsonAny::str("livestream"));
    base->set("prdline", SrsJsonAny::str("live"));
    base->set("cpu", SrsJsonAny::number(u->percent * 100)); //单位：%
    base->set("menory", SrsJsonAny::number(memory)); //单位：Mb
    printf("%s\n", base->dumps().c_str());

    print_stream_info(); //打印推拉流信息
